...
function teamInitials(team) {
  return team.abbreviation || team.name.split(" ").map((word) => word[0]).join("").slice(0, 3);
}

function badgeTextColor(hex = "#000B36") {
  const clean = hex.replace("#", "");
  if (clean.length !== 6) return "#FFFFFF";
  const r = parseInt(clean.slice(0, 2), 16);
  const g = parseInt(clean.slice(2, 4), 16);
  const b = parseInt(clean.slice(4, 6), 16);
  return (0.299 * r + 0.587 * g + 0.114 * b) / 255 > 0.66 ? "#000B36" : "#FFFFFF";
}
...
export default function TeamDirectory({ teams }) {
  ...
  const visibleTeams = useMemo(() => {
    ...
    return teams.filter((team) => {
      ...
      const searchMatch = !normalized || [team.name, team.city, team.nickname, team.abbreviation, team.arena, team.division]
        .filter(Boolean)
  ...
  const divisions = filters.slice(1);
  ...
  return (
    <div>
        ...
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          <label className="relative block w-full lg:max-w-sm">
            ...
            <input
              ...
              placeholder="Search team, city, arena…"
          ...
          <div className="flex gap-2 overflow-x-auto pb-1">
            {filters.map((filter) => (
              <button
                ...
                className={`whitespace-nowrap rounded-full px-4 py-2.5 text-xs font-black uppercase tracking-wide transition ${
                  division === filter
                    ? "bg-[#000B36] text-white"
                    : "border border-[#000B36]/12 bg-white text-[#000B36]/65 hover:border-[#18BDFC] hover:text-[#000B36]"
                }`}
              >
                {filter}
              </button>
      ...
      <p className="mt-6 text-sm font-bold text-[#000B36]/50">
        Showing {visibleTeams.length} of {teams.length} Major League clubs
      </p>
              ...
              <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
                {divisionTeams.map((team) => (
                  <Link
                    ...
                    className="group overflow-hidden rounded-3xl border border-[#000B36]/10 bg-white shadow-[0_8px_30px_rgba(0,11,54,0.05)] transition hover:-translate-y-1 hover:border-[#18BDFC]/70 hover:shadow-[0_18px_45px_rgba(0,11,54,0.11)]"
                  >
                    <div className="h-2" style={{ backgroundColor: team.colors.primary }} />
                    <div className="p-5">
                      <div className="flex items-center justify-between gap-3">
                        <span
                          className="flex h-12 w-12 items-center justify-center rounded-2xl border-2 text-sm font-black"
                          style={{ backgroundColor: team.colors.primary, borderColor: team.colors.secondary, color: badgeTextColor(team.colors.primary) }}
                        >
                          {teamInitials(team)}
                        </span>
                        <span className="text-xs font-black uppercase tracking-wider text-[#000B36]/35">{team.abbreviation}</span>
                      </div>
                      <p className="mt-5 text-xs font-black uppercase tracking-[0.18em] text-[#000B36]/40">{team.city}</p>
                      ...
                      <p className="mt-5 text-xs font-black uppercase tracking-wide text-[#A90117] transition group-hover:text-[#000B36]">View club →</p>
                    </div>
                  </Link>
                ))}
              </div>
      ...
      {visibleTeams.length === 0 && (
        <div className="mt-12 rounded-3xl border border-dashed border-[#000B36]/20 bg-[#F6F8FC] p-10 text-center">
          ...
          <button onClick={() => { setQuery(""); setDivision("All"); }} className="mt-3 text-sm font-black text-[#A90117]">
            Clear filters
          </button>
        </div>
      )}
