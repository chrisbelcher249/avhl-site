...
function MinorTeamCard({ team }) {
  return (
    <Link
      href={`/minor-league/${team.slug}`}
    ...
    >
      ...
      <div className="p-5">
        ...
        <div className="mt-5 border-t border-[#000B36]/8 pt-4">
          ...
          <p className="mt-3 text-xs font-black uppercase tracking-wide text-[#A90117] transition group-hover:text-[#000B36]">View club →</p>
        </div>
      </div>
    </Link>
  );
      ...
      {division === "All" ? (
        <div className="mt-8">
          <div className="mb-5 flex items-end justify-between gap-4 border-b border-[#000B36]/10 pb-4">
            <div>
              <p className="text-xs font-black uppercase tracking-[0.24em] text-[#A90117]">Minor League Directory</p>
              <h2 className="mt-1 text-3xl font-black tracking-tight md:text-4xl">All affiliates A–Z</h2>
            </div>
            <span className="hidden text-sm font-bold text-[#000B36]/40 sm:block">{visibleTeams.length} teams</span>
          </div>

          <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
            {visibleTeams.map((team) => <MinorTeamCard key={team.abbreviation} team={team} />)}
          </div>
        </div>
      ) : (
          ...
          {filters.slice(1).map((divisionName) => {
            ...
            return (
              <section key={divisionName} id={divisionName.toLowerCase()} className="scroll-mt-36">
                ...
                <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
                  {divisionTeams.map((team) => <MinorTeamCard key={team.abbreviation} team={team} />)}
                </div>
              </section>
            );
          })}
