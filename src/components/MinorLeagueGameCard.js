...
const minorTeamBySlug = Object.fromEntries(minorTeams.map((team) => [team.slug, team]));

function TeamRow({ slug, venue, score, winner, focusTeamSlug }) {
  const team = minorTeamBySlug[slug];
  if (!team) return null;
  ...
  return (
    <div className={`flex items-center gap-3 rounded-2xl px-3 py-2.5 ${focused ? "bg-[#000B36]/[0.055]" : ""}`}>
      ...
      <Link href={`/minor-league/${team.slug}`} className="flex min-w-0 flex-1 items-center gap-3 rounded-xl outline-none transition hover:text-[#A90117] focus-visible:ring-2 focus-visible:ring-[#18BDFC]">
...
export default function MinorLeagueGameCard({ game, focusTeamSlug = null }) {
  const complete = Number.isFinite(game.awayScore) && Number.isFinite(game.homeScore) && game.awayScore !== game.homeScore;
