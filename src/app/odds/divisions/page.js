...
export const metadata = {
  title: "Division Futures",
  description: "AVHL division-title probabilities generated from full-season Monte Carlo simulations.",
};

export const dynamic = "force-dynamic";
export const revalidate = 0;

export default async function DivisionOddsPage() {
    ...
    <main className="min-h-screen bg-slate-50 text-[#000B36]">
      <section className="bg-[#000B36] text-white">
        <div className="mx-auto max-w-7xl px-5 py-12 md:px-8 md:py-16">
          ...
          <h1 className="mt-3 text-4xl font-black tracking-tight sm:text-5xl">Division Futures</h1>
          <p className="mt-4 max-w-3xl text-base leading-7 text-white/65 md:text-lg">Probability each club finishes first in its division after simulating the entire remaining regular season.</p>
        </div>
      </section>
      <div className="mx-auto max-w-7xl space-y-6 px-5 py-8 md:px-8 md:py-10">
        ...
        <OddsFuturesBoard mode="divisions" schedule={schedule} teams={teams} />
      </div>
    </main>
  );
}
